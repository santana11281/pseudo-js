/**
 * PSeudo (PS1 emulator) – built browser bundle.
 *
 * What this file contains
 * - A complete PlayStation (PSX/PS1) emulator implemented in JavaScript.
 * - The code is organized as a set of factory functions on the `pseudo` namespace
 *   (e.g. `pseudo.CstrAudio`, `pseudo.CstrMips`). Each factory returns an object
 *   containing the public API for that subsystem.
 * - At the bottom of the file, the emulator creates singleton instances
 *   (e.g. `const audio = new pseudo.CstrAudio()`), which other subsystems use.
 *
 * How it is produced
 * - This is the build output generated by the repository `build` script:
 *   concatenates `src/*.h` + `src/*.js`, runs a C-preprocessor pass, then strips
 *   empty lines into `dist/pseudo.js`.
 *
 * Core subsystems (high level)
 * - Audio/SPU: ADPCM decode and WebAudio output.
 * - Bus/IRQ/DMA: interrupt scheduling and DMA start logic.
 * - CDROM: ISO reading and drive emulation.
 * - CPU (MIPS R3000A): instruction decode/execute + exceptions.
 * - COP2 / GTE: geometry transformation engine.
 * - Memory: RAM/ROM/scratchpad/hardware registers + typed views.
 * - GPU/Render: command processing and drawing.
 * - Timers/Counters, Serial (SIO), MDEC, texture cache, and misc I/O.
 *
 * Conventions used throughout
 * - Addresses and many registers are treated as unsigned 32-bit. Code often uses
 *   bitwise ops (e.g. `>>> 0`) to keep values in 32-bit range.
 * - Memory arrays are typed arrays with shared backing buffers. See `union()`.
 * - Many `scopeR/scopeW` methods implement memory-mapped register reads/writes.
 */

// Preprocessor
// A helper for creating multiple typed views over the same block of memory.
/**
 * Creates a shared ArrayBuffer and exposes multiple typed-array views over it.
 *
 * This is used to model PSX memory/register regions where the same bytes may be
 * accessed as 8-bit, 16-bit, or 32-bit (signed/unsigned) values.
 *
 ...
 *   sh: Int16Array,
 *   sb: Int8Array
 * }} Typed views sharing a single backing buffer.
 */
function union(size) {
 ...
 *
 * Responsibilities
 * - Emulates the PSX SPU voice registers that this emulator uses (pitch, volume,
 *   start address, loop address).
 * - Decodes PSX ADPCM blocks into PCM samples.
 * - Mixes all active voices into a stereo buffer.
 * - Streams audio to the browser using the Web Audio API.
 *
 * Notes
 * - This implementation focuses on the subset needed by many homebrew/demo
 *   content; not every SPU feature is emulated.
 * - Several `scopeW/scopeR` register cases intentionally pass through to
 *   `mem.hwr` (hardware register mirror) for simplicity.
 */
pseudo.CstrAudio = function() {
    ...
    let sbuf;
    /**
     * Converts a Uint16Array-like buffer containing 16-bit PCM samples into
     * Float32 samples suitable for WebAudio.
     *
     * WebAudio expects -1.0..+1.0 float samples. The SPU mixer here produces
     * 16-bit signed PCM stored in a Uint16 array; this converts with sign fixup.
     *
     * @param {Uint16Array} input Interleaved/stereo buffer of 16-bit values.
     * @returns {Float32Array} Float32 PCM samples for WebAudio.
     */
    ...
    /**
     * Decodes an SPU volume register value into a signed gain scalar.
     *
     * The PSX SPU volume registers use a sign/magnitude-like encoding. This
     * maps it into a signed 15-bit-ish range used by the mixer.
     *
     * @param {number} data 16-bit register value.
     * @returns {number} Signed volume value.
     */
    function setVolume(data) {
        return ((data & 0x7fff) ^ 0x4000) - 0x4000;
    }
    /**
     * Starts voices whose bits are set in the SPU key-on register.
     *
     * @param {number} data Bitmask of voices to start (one bit per channel).
     */
    ...
    /**
     * Stops voices whose bits are set in the SPU key-off register.
     *
     * @param {number} data Bitmask of voices to stop (one bit per channel).
     */
    ...
    /**
     * Produces the next stereo block of SPU samples.
     *
     * This:
     * - Clears the mix buffer.
     * - Initializes any newly-started channels.
     * - Decodes/mixes each active voice into the shared buffer.
     *
     * @returns {Uint16Array} Interleaved stereo samples (L block then R block).
     */
    ...
    /**
     * Decodes and mixes a single SPU voice (channel) into the shared mix buffer.
     *
     * The PSX SPU stores audio in 16-byte ADPCM blocks which expand to 28 PCM
     * samples. This method:
     * - When needed, reads the next ADPCM block, applies predictor/shift,
     *   handles loop/end flags, and fills `ch.bfr` with 28 decoded samples.
     * - Advances the channel's fractional position (`spos`) using `freq`.
     * - Mixes samples into left/right outputs with volume scaling.
     *
     * @param {object} ch Voice state object from `spuVoices[n]`.
     */
    ...
    return {
        /**
         * Initializes SPU memory, per-voice state, and the WebAudio pipeline.
         *
         * Must be called once before any audio output can occur.
         */
        init() {
        ...
        /**
         * Resets SPU state to power-on defaults.
         *
         * Clears SPU RAM, resets transfer address, clears all voices, and
         * reconnects the ScriptProcessor to begin output.
         */
        reset() {
            spuMem.uh.fill(0);
            spuAddr = 0xffffffff;
        ...
        /**
         * Memory-mapped SPU register write handler.
         *
         * The emulator routes writes in the SPU I/O region here. This updates
         * per-voice registers (volume, pitch, start address, loop address),
         * handles key-on/key-off, and implements SPU RAM transfers.
         *
         * @param {number} addr SPU register offset (typically relative to 0x1F801C00).
         * @param {number} data 16-bit value written by the CPU.
         */
        scopeW(addr, data) {
            switch(true) {
        ...
        /**
         * Memory-mapped SPU register read handler.
         *
         * Many reads are served from the mirrored `mem.hwr` register array;
         * a few special cases return derived/emulator-specific values.
         *
         * @param {number} addr SPU register offset.
        ...
        /**
         * Executes an SPU DMA transfer.
         *
         * Uses DMA channel register values in `mem.hwr` to determine transfer
         * direction and size, and copies data between main RAM and SPU RAM.
         *
         * @param {number} addr DMA channel base address.
         */
        executeDMA(addr) {
